...
- The [openLinks](./README.md#openlinks) extension now also supports external links and wiki links in comments
...
- Potential TypeError from [`setColumnGuide`](./README.md#setcolumnguide)
...
- New extension option: [stickyScroll](./README.md#stickyscroll)
...
- The [colorPicker](./README.md#colorpicker) extension now also supports named CSS colors in `style` attributes in the MediaWiki mode
...
- [Autocompletion](./README.md#autocompletion) for the `src` attribute of `<templatestyles>` tags in the MediaWiki mode and Lua functions `require`, `mw.loadData` and `mw.loadJsonData` in the Lua mode on a MediaWiki site
...
- Use [`mw.Message#parse`](https://doc.wikimedia.org/mediawiki-core/master/js/mw.Message.html#parse) to sanitize HTML strings generated by [Extension:TemplateStyles](https://www.mediawiki.org/wiki/Extension:TemplateStyles) on a WMF-hosted site
