...
class MediaWiki {
	...
	eatList(stream: StringStream, state: State): string {
		...
		if (mt && mt[0].includes(';')) {
			if (state.nDt === 0) {
				state.nTemplateDt = state.nTemplate;
				state.nLinkDt = state.nLink;
				state.nExtDt = state.nExt;
			}
	...
	inExternalLink(stream: StringStream, state: State): string {
		...
		} else if (stream.eatSpace() || stream.match(/^(?:[[<>"]|&[lg]t;|'{2,3}(?!')|'{5}(?!')|\{{2,})/u, false)) {
	...
	eatParserFunctionName(chars: number): Tokenizer {
		return (stream, state) => {
			...
			state.tokenize = this.inParserFunctionColon(!stream.eol());
	...
	eatWikiText(style: string): Tokenizer {
		return (stream, state) => {
			...
			switch (ch) {
				...
				/** @todo consider HTML tag balance */
				case ':':
					if (
						state.nDt
						&& state.nTemplateDt === state.nTemplate
